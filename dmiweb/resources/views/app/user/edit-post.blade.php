            ...
            <div class="widget">
              <div class="box-head">
                <div class="titlebox title-arrow-right">
                  <h2 class="title"><span><i class="ion-edit"></i> EDIT ARTICLE</span></h2>
                </div>
               </div>{{-- .box-head--}}
               {!! Form::model($post,['method'=> 'PATCH','action'=>['FrontendController@updatepost',$post->id], 'enctype' => 'multipart/form-data']) !!}
                         ...
                         <div class="form-group {{ $errors->has('image_upload') ? ' has-error' : '' }} allmedia">
                              @if (count($post->medias))
                                <label>Current Image</label>
                                 ...
                                 <div class="rows flex-box">
                                   @foreach($post->medias as $image)
                                   <div class="media-{{$image->pivot->id}}">
                                     <div class="img-preview text-center">
                                     <img class="media-{{$image->id}}" src="{{ url('/') }}/media/{{$image->id}}/conversions/small.jpg" alt="" />
                                      <a href="#" class="deleteImage" data-token="{{ csrf_token() }}" media-id="{{$image->id}}"  data-id="{{ $image->pivot->id }}"><i class="fa fa-trash"></i></a>
                                     </div>
                                   </div>
                                   @endforeach
                                 </div>
